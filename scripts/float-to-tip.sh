...
[ -n "$TARGET" ] || [ -n "$GREP" ] || { sed -n '2,15p' "$0"; exit 1; }
...
if [ -n "$GREP" ]; then
  ...
  [ "$COUNT" = 1 ] || { echo "refuse: --grep '$GREP' matched $COUNT commits" >&2; git log --oneline -i --grep="$GREP" "$BASE..HEAD" >&2; exit 1; }
  TARGET="$MATCHES"
fi
TARGET=$(git rev-parse --verify --quiet "$TARGET^{commit}") || { echo "refuse: unknown commit" >&2; exit 1; }
git merge-base --is-ancestor "$TARGET" HEAD || { echo "refuse: commit is not on this branch" >&2; exit 1; }
git merge-base --is-ancestor "$TARGET" "$BASE" && { echo "refuse: commit is on $MAIN" >&2; exit 1; }
...
if ! GIT_SEQUENCE_EDITOR="$SELF --todo $TARGET" GIT_EDITOR=: git rebase -q -i ${AUTOSTASH[@]+"${AUTOSTASH[@]}"} "$BASE" 2>"${TMPDIR:-/tmp}/float-to-tip.err"; then
