...
- `pre:<pass>` — launched **alone, before** the rest of the batch, which waits for its
  verdict (currently unused; every pass runs in the one batch).
...
| fix | agent-skills:fix-reviewer | both | the behavior the fix changes |
...
| self | fix | full | general+fit tests slop fix | 5 |
| self | fix | lite | general+fit tests slop fix | 5 |
| self | fix | trivial | general+fit+tests+slop fix | 5 |
...
| pr | fix | any | general tests slop fit fix | 0 |
...
- A dropped pass never drops its **question** — it folds into general. What no tier
  drops is the type's defining pass: requirements on a feature, the fix pass (premise
  and root cause) on a fix, behavior preservation on a refactor.
...
- `fix` is identical at lite and full: four agents is already the lean pipeline, and the
  fix pass's premise procedure is at its most valuable on a tiny fix — a flipped guard is
  exactly where a recorded decision hides. The procedure runs cheapest-first inside the
  pass and answers premise before root cause, so a contradicted premise still costs almost
  nothing extra.
...
- Lanes, not pass count, are what a profile pays for. Tests are usually most of a
  ...
  once per pass to review them once. `requirements`, `fix`, `behavior` and `tests`
